...
//const resolution = "720p"; // Can be "1080p" or "720p"aconst ffmpeg = require('fluent-ffmpeg');
const fs = require('fs');
const fsp = fs.promises;
const path = require('path');
const https = require('https');
  ...
  try {
    const promises = Array(numRequests).fill().map(async (_, index) => {
      ...
      try {
        ...
        // Increased delay to 30000ms (30 seconds) after each successful generation
        await new Promise(resolve => setTimeout(resolve, 30000));
        console.log(`Cooling down after request ${index + 1} for 30 seconds...`);
        ...
        // Increased delay to 30000ms even after failed attempts
        await new Promise(resolve => setTimeout(resolve, 30000));
        console.log(`Cooling down after failed request ${index + 1} for 30 seconds...`);
...
// Remove the standalone batch processing code at the bottom and add this async function
async function processBatches(promises) {
  const batchSize = 3; // Reduced batch size
  const results = [];
